...
var $nav = document.querySelector('.nav'); // render

function generateId(content) {
  id = content.length ? Math.max.apply(Math, _toConsumableArray(content.map(function (todo) {
    return todo.id;
  }))) + 1 : 1;
}
...
function render(content) {
  ...
  $completedTodos.innerHTML = _toConsumableArray(content.filter(function (todo) {
    return todo.completed;
  })).length;
  $activeTodos.innerHTML = _toConsumableArray(content.filter(function (todo) {
    return !todo.completed;
  })).length; // console.log();
...
function addItem(e) {
  if (e.keyCode !== 13 || $inputTodo.value.trim() === '') return; // todos = [{ id: generateId(), content: $inputTodo.value, completed: false }, ...todos];
